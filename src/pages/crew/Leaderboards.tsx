...
export default function Leaderboards() {
  ...
  const ranked = [...mockUsers].sort((a, b) => {
    const av = get(a, active.field) ?? 0;
    const bv = get(b, active.field) ?? 0;
        ...
        {defaultCategories.map(c => (
          <button
            ...
            className={`px-3 py-1 rounded-full text-sm ${
              active.key === c.key ? "bg-amber-500 text-slate-900" : "bg-slate-800 text-slate-200"
            }`}
      ...
      <div className="grid grid-cols-3 gap-3 items-end h-56 mb-4">
        {top3.map((p, i) => (
          <div key={p.id} className="flex flex-col items-center">
            <div
              className="w-20 bg-amber-600 rounded-t-2xl shadow-lg"
              style={{ height: [120, 160, 100][i] }}
            />
